...
class Generator(nn.Module):
    def __init__(self, config):
        ...
        num_residual_blocks = config['model'].get('num_residual_blocks', 9)
        num_channels = config['data'].get('num_channels', 3)
        
        # Encoder
        self.conv_input = nn.Conv2d(num_channels, num_filters, 7, padding=3)
        ...
        self.residual_layers = nn.Sequential(*[
            ResidualBlock(num_filters * 4) for _ in range(num_residual_blocks)
        ])
        ...
        # Output
        self.conv_output = nn.Conv2d(num_filters, num_channels, 7, padding=3)
